...
export default async (req, res) => {
    try {
        ...
        const language = req.acceptsLanguages("en", "en-US", ...availableLocales) || "en";
        const locale = (language === "en-US" && "en") || language;
        let languagePack;

        try {
            languagePack = {
                locale,
                messages: (availableLocales.indexOf(locale) === -1 && {}) || await import(`app/translations/locales/${locale}.json`)
            };
        } catch (reason) {
            languagePack = {
                locale
